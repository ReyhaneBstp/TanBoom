...
export function OptionCard({ title, description, icon, selected, onClick, className, children }: OptionCardProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "group relative flex min-h-24 w-full flex-col items-start justify-between overflow-hidden rounded-[1.6rem] border border-white/75 bg-white/45 p-4 text-right backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-rose-300/70 hover:bg-white/75 hover:shadow-soft-rose",
