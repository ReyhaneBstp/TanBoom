...
import type { FabricOption, PatternedFabric, SolidFabric } from "@/types/design";

const isSolidFabric = (fabric: FabricOption): fabric is SolidFabric =>
  fabric.kind === "solid";
const isPatternedFabric = (fabric: FabricOption): fabric is PatternedFabric =>
  fabric.kind === "patterned";

export function StepFabric() {
  const activeFabricKind = useDesignStore((s) => s.activeFabricKind);
  const selectedFabricIds = useDesignStore((s) => s.selectedFabricIds);
  const setActiveFabricKind = useDesignStore((s) => s.setActiveFabricKind);
  ...
  return (
    <div className="space-y-5">
        ...
        {[
          ...
          return (
            <button
              key={tab.id}
              type="button"
              onClick={() => setActiveFabricKind(tab.id)}
              className={cn(
                "flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold transition",
                activeFabricKind === tab.id
                  ? "bg-white text-rose-600 shadow-soft-rose"
          ...
          {patternedFabrics.map((fabric) => {
            ...
            return (
              ...
              >
                ...
                <span className="mt-2 block px-1 text-xs font-semibold text-foreground">
                  {fabric.label}
                </span>
              </button>
            );
      ...
      <p className="rounded-[1.3rem] border border-white/70 bg-white/40 px-4 py-3 text-xs leading-6 text-muted-foreground backdrop-blur-xl">
        می‌توانید چند پارچه را هم‌زمان انتخاب کنید؛ رنگ‌های ساده با کد Hex و
        پارچه‌های طرح‌دار با داده تصویر به پرامپت افزوده می‌شوند.
      </p>
    </div>
  );
}